  ...
  render() {
    return (
      ...
      <div className="button-container">
        <button class="btn btn-warning" id="button" onClick={(event)=>this.props.handleSelectCategory(event)}>Education</button>
        <button class="btn btn-warning" id="button"onClick={(event)=>this.props.handleSelectCategory(event)}>Healthcare</button>
        <button class="btn btn-warning" id="button"onClick={(event)=>this.props.handleSelectCategory(event)}>Environment</button>
      </div>
      <div>
        {this.props.selectedCategory ? <h1>{this.props.selectedCategory}</h1> :null}
        {this.props.selectedCategory ? <button class="btn btn-success" onClick={this.props.handleSubmitVote}>Submit</button> :null}
      </div>
      </div>
    );
  }
